...
#define G_HUGE_BUFFER 524288 //!< Most reads/writes to Galil hardware are small. This value will hold the largest array or program upload/download possible.
  ...
  *
	...
	*      * `memory1` is output. If nonnull, value must be an `unsigned int*` which will be filled with the current @ref gcaps timeout, in ms.
	*      * `memory2` is input. If nonnull, value must be an `unsigned int*` holding the new @ref gcaps timeout, in ms, for connection `g`.
	*
