...
#define POLLINGINTERVAL 100 //!< Interval, in milliseconds, for polling commands, e.g. GMotionComplete().
#define G_USE_GCAPS //!< Use the GCAPS server in GAddresses(), GAssign(), GIpRequests(), and GVersion(). To avoid GCAPS, comment out this line and recompile, http://galil.com/sw/pub/all/doc/gclib/html/gclibo.html
	...
	*
	*  \return The success status or error code of the function. If the options parameter is set to 0, the return value will be a bit mask indicating which sectors in the specified GCB are not empty. Otherwise, see gclib_errors.h for possible error values.
	*
	*  \note By default, GSetupDownloadFile() will stop immediately if an error is encountered downloading data. This can be overridden in the options parameter. An example of when you might override this setting: You have a backup from an 8-axis controller and want to restore the parameters for the first 4 axes to a 4-axis controller.
	*
